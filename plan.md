...
| 2026-08-13 | 方向键常显 bug 修复完成：默认隐藏生效（opacity 0 / 不可点击），旧版 toggleDpad 已删除，本地验证通过 | 2026-08-13 | 已完成 |
| 2026-08-13 | 修复方向键遮挡游戏画面：方向键弹出时游戏画面整体上移（画布不缩小），收起时恢复居中；移动距离按屏幕动态计算并限制不越出顶部 | 2026-08-13 | 待开始 |
| 2026-08-13 | 方向键遮挡修复完成：弹出时游戏卡片动态上移避开方向键、收起恢复居中，0.2s 过渡，画布不缩小 | 2026-08-13 | 已完成 |

### 当前状态

- v0.1.0 已上线：三层网站框架 + 贪吃蛇，网址 https://illheartdark.github.io/text/（手机 / 电脑均可直接游玩）
- 待办：无（方向键遮挡问题已于 2026-08-13 修复完成并部署）
...
- **原因**：方向键为屏幕底部悬浮层，弹出后覆盖游戏画布底部。
- **修改**：
  - `play.js`：打开方向键时动态测量画布位置，将游戏卡片整体上移（画布大小不变）避开方向键；收起时恢复居中；上移范围限制在顶部标题之下。
  - `style.css`：游戏卡片移动加 0.2s 平滑过渡动画。
- **验证**：手机端弹出方向键后棋盘完整可见不被遮挡；收起后恢复居中；画布不缩小；触控滑动与桌面键盘不受影响。
- **结果**：2026-08-13 修复完成——play.js 动态上移（窗口变化重算）、style.css 0.2s 过渡，12 项测试通过并已部署线上。
