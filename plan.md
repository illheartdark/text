...
| 2026-08-15 | 设置面板 UI 重构与账户体系已上线：返回按钮常驻 + 标题居中、面板去毛玻璃改卡片式、账户大卡片（头像+昵称+ID）、账户点击弹浮层（手机底部弹出/电脑居中弹窗）、本地模拟登录（内测账号 00001/00001，昵称「内测版」）、Account Provider 接口抽象（为 App / 小程序预留）；30 项测试通过；合并 feat/settings-ui 推送部署成功 | 2026-08-15 | 已完成·已上线 |
...
- 设置面板 UI 重构与账户体系 v0.2.2 已上线：返回按钮常驻 + 标题居中、去毛玻璃卡片 UI、账户大卡片 + 弹层、Account Provider 接口（内测账号 00001 / 00001，昵称「内测版」）；30 项测试通过（方案见文末）
