...
| 2026-08-13 | 修复方向键遮挡（新办法）：方向键改为页面流式布局（不再悬浮覆盖），打开时让出底部空间、画布完整可见；上版 transform 方案已回退 | 2026-08-13 | 待开始 |
...
- **新办法：方向键改为页面流式布局，不再悬浮覆盖**：
  - `play.html`：方向键移出游戏卡片，作为 play-shell 内卡片下方的独立元素。
  - `style.css`：方向键去掉 fixed / left / bottom 悬浮定位；默认 display:none，打开时 display:grid（带淡入缩放动画）；游戏卡片去掉 transform 过渡，玻璃效果恢复正常。
  - `play.js`：开关只切换 class（不再测量位置、不再用 transform）；方向键占据布局底部空间，卡片在剩余空间自动重新居中，画布天然位于方向键上方、永不被遮挡。
- **验证**：手机端打开方向键后棋盘完整可见；收起恢复；玻璃效果正常；滑动触控与桌面键盘不受影响。
